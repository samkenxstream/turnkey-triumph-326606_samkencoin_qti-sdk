    ...
    public function render($document)
    {
        ...
        if ($this->isTemplateOriented() === true) {
            ...
            if (($c = preg_match_all('/<!--\s+(?:qtism-include)\s*\((?:(\$.+?), ([0-9]+), "(.+?)", ([0-9]+?))\)\s*:\s+-->(.+?)<!--\s+qtism-endinclude\s+-->/ius', $output, $matches)) > 0) {
                ...
                for ($i = 0; $i < $c; $i++) {
                    $output = str_replace($matches[0][$i], '<?php include(__DIR__ . "/' . $fragmentPrefix . $matches[2][$i] . '-" . ' . $matches[1][$i] . '->getShuffledChoiceIdentifierAt(' . $matches[2][$i] . ', ' . $matches[4][$i] . ') . ".phtml"); ?>', $output);
                    $fragments[] = [
